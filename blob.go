...
func (blob *Blob) Get(fieldName string) []byte {
	...
	if !ok {
		panic(fmt.Sprintf("Invalid field name: %s", fieldName))
	}
...
func (blob *Blob) From(startFieldName string) []byte {
	...
	if !ok {
		panic(fmt.Sprintf("Invalid field name: %s", startFieldName))
	}
...
func (blob *Blob) To(endFieldName string) []byte {
	...
	if !ok {
		panic(fmt.Sprintf("Invalid field name: %s", endFieldName))
	}
